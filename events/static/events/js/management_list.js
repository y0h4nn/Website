...
(function(){
    ...
    var UserListFOR = function(containerId, listProvider, buildCallback){
        BaseList.call(this, containerId, buildCallback);
        this.searchInput.id = "search_input";
        queryJson(listProvider, {}, this.populate.bind(this));
    };

    UserListFOR.prototype = Object.create(BaseList.prototype, {
        populate: {
            value: function(json){
                // Registrations
                this.elems = json['reg'].concat(json['ext_reg']).concat(json['invits']);
                ...
                for(var i in this.elems){
                    ...
                    pictureContainer.appendChild(img);
                    user.element.addEventListener("click", (function(){
                        if(this.klass == "bg-green" || this.klass == "" || this.klass == "bg-red"){  // He needs to pay.
                            if(this.formula){
                                var txt = this.display_name + " - " + this.formula + " (" + this.formula_price + " €)";
                            }
                            else{
                                var txt = this.display_name;
                            }
                            document.popup = new SelectionPopup(txt, payment_means, function(choice){
                                queryJson('nl_ack', {'type': this.user.type, 'eid': event_id, 'iid': this.user.id, 'user': this.user.user, 'payment_mean': choice}, function(resp){
                                    if(resp['error']){
                                        add_message('error', resp['error']);
                                    }
    ...
    window.UserListFOR = UserListFOR;
    window.UserListFOR.UserAction = Action;
