...
var BaseList = function(containerId, buildCallback){
    ...
    this.updateElems = function(){
        var pattern = removeDiacritics(this.searchInput.value).split('').join('.*?');
...
Object.defineProperty(BaseList.prototype, "match", {
    writable: true,
    value: function(user){
        var matching = false;
        var regex = this.cachedSearchRegex;
        matching |= regex.test(removeDiacritics(user['last_name']));
        matching |= regex.test(removeDiacritics(user['first_name']));
        matching |= regex.test(removeDiacritics(user['nickname']));
        matching |= regex.test(removeDiacritics(user['username']));
        matching |= regex.test(removeDiacritics(user['display_name']));
        return matching;
    }
});
